...
interface AssignmentListByEmployeeProps {
  employeeId: string;
}

export function AssignmentListByEmployee({ employeeId }: AssignmentListByEmployeeProps) {
  const { role } = useAuth();
            ...
            cell: ({ row }: { row: { original: PolicyAssignment } }) =>
              !row.original.effective_to ? (
                <ConfirmDialog
                  ...
                  description="This will set today as the effective end date for this assignment."
  ...
  if (!isLoading && data?.items.length === 0) {
    return <EmptyState title="No assignments" description="This employee has no policy assignments." />;
  }
